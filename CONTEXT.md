...
**Date:** 2026-08-16  
**Status:** Shipping landing + Free-ticket fixes to **prod (`main`)**. Man/woman (the sermon-adjacent gender path) stays **preview-only** — do not merge `6a13984` to main. Stripe cutover still parked: Production checkout is **Eco Delight Live**. Gender/man-woman stays **preview-only**.  
**Feedback videos:** always check `~/Desktop/Stuff/Lemon Voice/The Train Station/App Feedback Video/`. A new drop means **at least one issue**. John will say when the video is perfect — do not close the path before that. Latest (2026-08-16, not perfect): `ScreenRecording_08-16-2026 13-24-56_1.MP4` + `13-27-20_1.MP4` — signed-out Safari Free Tour → tickets → Free Explorer. Local fix in progress (no iOS YouTube chrome, full-bleed 5s gag then Jeremy, overlay chrome hidden). Not shipped; wait for a new recording after deploy.  
