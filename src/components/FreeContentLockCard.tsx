    ...
    <div className="card space-y-4 border-amber-500/40 bg-gradient-to-br from-amber-500/10 via-[var(--surface)] to-[#1a1030] p-5">
      ...
      {access.dayInCycle != null ? (
        <p className="text-xs text-[var(--muted)]">
          Today is day <span className="font-semibold text-[var(--text)]">{access.dayInCycle}</span> of{" "}
          {access.cycleDays}. Climb the Free division board — top band can claim a free week of{" "}
          {access.upgradeLabel || "Coach Class"}.
        </p>
      ) : null}
      <div className="flex flex-wrap gap-2">
        <Link href="/member/checkout?plan=member" className="btn-primary">
